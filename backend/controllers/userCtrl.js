...
exports.signup = async (req, res, next) => {
  try {
    ...
    if (emailVerifBD || pseudoVerifBD) {
      return res
        .status(400)
        .json({ message: "Compte déjà crée, merci de vous enregistrer. " });
    }
    bcrypt
      ...
      .then((valid) => {
        if (!valid) {
          return res.status(401).json({
            error: "Les informations renseignées ne sont pas correctes!",
          });
        }
        res.status(200).json({
          userId: userDB.id,
    ...
    if (tokenDB.id == userDB.id || tokenDB.roles == 1) {
      ...
      userDB
        ...
        .then(() => {
          return res
            .status(201)
            .json({ message: "Compte utilisateur modifié" });
        })
        .catch((err) => {
